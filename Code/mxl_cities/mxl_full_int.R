### Estimate conditional logit model to check interactions ###
rm(City, garden, education)
database <- database_merged
database <- database %>% filter(!is.na(Income_Dis_Present))

database <- database %>% mutate(Income_mean = as.numeric(scale(Income_Dis_Present, scale=FALSE)),
                                Age_mean = as.numeric(scale(Age, scale = FALSE)),
                                FlatSize_mean = as.numeric(scale(FlatSize, scale = FALSE)))
...
mxl_full_int = apollo_estimate(apollo_beta, apollo_fixed,
...
#### MODEL OUTPUTS                                               ##
# ################################################################# #
apollo_saveOutput(mxl_full_int)